// Shared implementation of the HubSpot property bootstrap.
// Behaviour is unchanged from the original route handler — it was moved here
// so both the HTTP route and the admin server function can call it in-process.
...
/** Read a single contact property definition straight from HubSpot. */
export async function readContactProperty(name: string) {
  const res = await fetch(`${GATEWAY}/crm/v3/properties/contacts/${name}`, {
    method: "GET", headers: hsHeaders(),
  });
  if (!res.ok) {
    return { ok: false as const, status: res.status, error: await res.text() };
  }
  return { ok: true as const, property: (await res.json()) as {
    name: string; type: string; fieldType: string;
